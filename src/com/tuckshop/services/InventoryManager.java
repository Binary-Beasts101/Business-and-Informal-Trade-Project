...
public class InventoryManager {
    ...
    public void displayReceipt() {
        ...
        for (Product item : stockList) {
            // POLYMORPHISM: Calling the abstract method from the Product reference
            System.out.println(item.getDisplayInfo() + " | Price: P" + String.format("%.2f", item.getPrice()));
        }
    ...
    // NEW METHOD: Added to satisfy the 4th user action requirement
        ...
        for (Product item : stockList) {
            if (item.getId().equalsIgnoreCase(id)) {
                if (amount > 0) {
                    item.setQuantity(item.getQuantity() + amount);
                    System.out.println("\n--- RESTOCK SUCCESSFUL ---");
                    System.out.println("Updated " + item.getName() + " stock to: " + item.getQuantity());
                } else {
